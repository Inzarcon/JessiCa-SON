"""Module containing ConfigObserver and ConfigObserverSavable base classes."""
...
class ConfigObserver:
    """Interface for classes that observe settings from a ConfigManager.

    Observes settings given in the constructor and auto-updates them on changes once registered to a ConfigManager.
    ...
    _config: dict[str, str | int | bool]
    _setting_to_func: dict[str, Callable] | None

    def __init__(self, config: dict[str, str | int | bool]) -> None:
        """Create ConfigObserver. Pass dictionary with setting names as keys and default setting values as values."""
        self._config = config
        self._setting_to_func = None
        for setting_name, value in config.items():
            self.config_notify(setting_name, value)
    ...
    def set_setting_to_func(self, setting_to_func: dict[str, Callable], *, keep_attr: bool = False) -> None:
        ...
        config; otherwise raises ValueError. The corresponding instance attribute created during initialization is also
        deleted unless keep_attr=True.
        """
    ...
    def _set_redirect_method(self, redirect: dict[str, Callable], name: str, *, keep_attr: bool = False) -> None:
        ...
        invalid = [setting_name for setting_name in redirect if setting_name not in self._config]
        if invalid:
            msg = f"All setting names in {name} must exist in config, but these do not: {invalid}"
    ...
    def config_notify(self, setting_name: str, value: str | int | bool | None) -> None:
        """Notify about configuration update and apply new settings. If value is None, reset back to default value."""
        if value is None:
            value = self._config.get(setting_name)

        if self._setting_to_func and setting_name in self._setting_to_func:
            func = self._setting_to_func.get(setting_name)
            if not callable(func):
                msg = f"""Not callable: {func}. Called late in config_notify; set_setting_to_func
    ...
    def register_at(self, config_manager: ConfigManager) -> None:
        """Register self to a ConfigManager."""
        config_manager.register_observer(self, list(self._config.keys()))


class ConfigObserverSavable(ConfigObserver):
    ...
    def __init__(self, config: dict[str, str | int | bool]) -> None:
        """Create ConfigObserverSavable. Works the same way as Parent constructor."""
    ...
    def config_state(self, setting_name: str) -> str | int | bool:
        """Return the current configuration state of a given setting. Called by ConfigManager to update its settings."""
        if self._func_to_setting and setting_name in self._func_to_setting:
            func = self._func_to_setting.get(setting_name)
            if not callable(func):
                msg = f"""Not callable: {func}. Called late in config_state; set_func_to_setting
                should have caught this earlier. Did you directly overwrite the attribute?"""
                raise ValueError(msg)
            return func()
